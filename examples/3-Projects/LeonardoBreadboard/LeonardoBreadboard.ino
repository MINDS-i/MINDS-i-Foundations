...
void AutonoDrive(String& msg){
	...
	if((rightc = minds_i_sensors::getPing(pingPins[0])) != 0) right = rightc;
	if((leftc = minds_i_sensors::getPing(pingPins[2])) != 0) left = leftc;
	if((frontc = minds_i_sensors::getPing(pingPins[1])) != 0) front = frontc;
...
void lineFollow(){
	...
	drive.write(90);
	if(minds_i_sensors::QTI(QTIPins[0]) < threshold) steer.write(45);
	else if(minds_i_sensors::QTI(QTIPins[1]) < threshold) steer.write(90);
	else if(minds_i_sensors::QTI(QTIPins[2]) < threshold) steer.write(135);
}
//////////////////////////////////////////////////
