...
void AutonoDrive(String& msg) {
    ...
    if ((rightc = getPing(pingPins[0])) != 0)
        right = rightc;
    if ((leftc = getPing(pingPins[2])) != 0)
        left = leftc;
    if ((frontc = getPing(pingPins[1])) != 0)
        front = frontc;
...
void lineFollow() {
    ...
    drive.write(90);
    if (QTI(QTIPins[0]) < threshold)
        steer.write(45);
    else if (QTI(QTIPins[1]) < threshold)
        steer.write(90);
    else if (QTI(QTIPins[2]) < threshold)
        steer.write(135);
}
//////////////////////////////////////////////////
